...
export const Calendar: React.FC<CalendarProps> = ({ value, onChange, theme }) => {
  ...
  const [viewDate, setViewDate] = useState(() =>
    isDateValid(value) ? new Date(value) : new Date(),
  );

  // Sync view if value changes to a valid date using "adjust state based on props" pattern
  const [prevValue, setPrevValue] = useState<Date | undefined>(undefined);
  if (value !== prevValue) {
    setPrevValue(value);
  ...
  const handlePrevMonth = (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    setViewDate((prev) => {
      const d = isDateValid(prev) ? prev : new Date();
  ...
  return (
    ...
    >
      ...
      >
        {daysOfWeek.map((d) => (
          <div key={d} style={{ color: '#9ca3af', fontSize: '12px' }}>
            {d}
          </div>
        ))}
      </div>
