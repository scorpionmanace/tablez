...
describe('Table Context Menu', () => {
  ...
  it('shows context menu when enabled in settings', () => {
    ...
    const { container } = render(<Table data={data} columns={columns} settings={settings} />);
  ...
  it('triggers hideColumn action', () => {
    ...
    const { container } = render(
      <Table data={data} columns={columns} settings={settings} onColumnUpdate={onColumnUpdate} />,
    );
  ...
  it('executes custom actions', () => {
    ...
    const settings: TableSettings = {
      contextMenu: {
        enabled: true,
        items: [{ label: 'My Action', onClick: customAction }],
      },
    };
  ...
  it('triggers shortcut for custom action', () => {
    ...
    const settings: TableSettings = {
      contextMenu: {
        enabled: true,
        items: [{ label: 'Save', onClick: customAction, shortcut: 'Mod+S' }],
      },
    };
    ...
    const settings: TableSettings = {
      contextMenu: {
        ...
        items: [
          {
            label: 'Group',
            children: [{ label: 'Sub Item', onClick: vi.fn() }],
          },
        ],
      },
    };
  ...
  it('triggers header rename from context menu', async () => {
    render(
      <Table
        data={data}
        columns={columns}
        settings={{ contextMenu: { enabled: true, items: ['renameColumn'] } }}
      />,
    );
