...
const ContextMenuItemComponent: FC<{
  ...
  const [subMenuPosition, setSubMenuPosition] = useState<{ side: 'left' | 'right' }>({
    side: 'right',
  });

  // Calculate submenu position after it opens (layout effect pattern)
  useEffect(() => {
    if (isSubMenuOpen && subMenuRef.current) {
      const frame = requestAnimationFrame(() => {
        const rect = subMenuRef.current?.getBoundingClientRect();
        if (rect && rect.right > window.innerWidth) {
          setSubMenuPosition({ side: 'left' });
        } else {
          setSubMenuPosition({ side: 'right' });
        }
      });
  ...
  const label = isDefault ? DEFAULT_LABELS[item] : item.label;
  const icon = isDefault
    ? DEFAULT_ICONS[item]
    : typeof item.icon === 'string'
      ? DEFAULT_ICONS[item.icon]
      : item.icon;
  const shortcut = isDefault ? DEFAULT_SHORTCUTS[item] : item.shortcut;
  const children = isDefault ? undefined : item.children;
  const type = isDefault ? 'item' : (item.type ?? 'item');

  // Automatic disabling based on cell state
  const isCopyAction =
    item === 'copy' ||
    item === 'copyTableWithHeader' ||
    item === 'copyTableWithoutHeader' ||
    (typeof item !== 'string' &&
      (item.label === 'Copy' || (typeof item.icon === 'string' && item.icon === 'copy')));
  const isCutAction =
    item === 'cut' ||
    (typeof item !== 'string' &&
      (item.label === 'Cut' || (typeof item.icon === 'string' && item.icon === 'cut')));
  const isPasteAction =
    item === 'paste' ||
    (typeof item !== 'string' &&
      (item.label === 'Paste' || (typeof item.icon === 'string' && item.icon === 'paste')));

  let disabled = isDefault ? false : !!item.disabled;
  if (!disabled) {
    ...
    >
      ...
      {!!children && !!isSubMenuOpen && (
        <div
          ...
          style={{
            position: 'absolute',
            left: subMenuPosition.side === 'right' ? '100%' : 'auto',
            right: subMenuPosition.side === 'left' ? '100%' : 'auto',
            top: '-4px',
